...
double pEuler(LagrangianPoint *pt, double tstart, double tend) {
  ...
  ***/
  ...
  while((tc + TINY) < tend) { /* Integration not complete */
    ...
    if((Data_MeshType == CARTESIAN && !Data_XPeriodic && TestOutsideDomain(pt->X)) || (Data_MeshType == UNSTRUCTURED && (pt->ElementIndex < 0 || TestOutsideDomain(pt->X)))) { 
      /* Point left domain */
      pt->LeftDomain = 1;
...
double RK4(LagrangianPoint *pt, double tstart, double tend) {
  ...
  ***/
  ...
  while((tc - tend) * (tend - tstart) < 0.0 ) {
    ...
    /* Check if point left domain */
    if((Data_MeshType == CARTESIAN && !Data_XPeriodic && TestOutsideDomain(pt->X)) || (Data_MeshType == UNSTRUCTURED && pt->ElementIndex < 0)
